...
const ParticleBackground = (props: any) => {
  ...
  const boundCheckSettings = () => {
    if (settingsWithDefaults.opacity.maxOpacity > 1 || settingsWithDefaults.opacity.maxOpacity < 0)
      settingsWithDefaults.opacity.maxOpacity = 1;
    if (settingsWithDefaults.opacity.minOpacity < 0 || settingsWithDefaults.opacity.minOpacity > 1)
      settingsWithDefaults.opacity.minOpacity = 0;
    if (settingsWithDefaults.opacity.opacityTransitionTime < 0)
      settingsWithDefaults.opacity.opacityTransitionTime = 1000;
  };
  ...
  const drawParticles = () => {
    ...
    for (let i in particles) {
      ...
      if (settingsWithDefaults.canvas.useBouncyWalls) {
        ...
        if ((p.x - p.size < 0 && p.vx < 0) || (p.x + p.size > ctx.canvas.width && p.vx > 0))
          p.x = p.getRandomInRange(p.size, ctx.canvas.width - p.size);
        if ((p.y - p.size < 0 && p.vy < 0) || (p.y + p.size > ctx.canvas.height && p.vy > 0))
          p.y = p.getRandomInRange(p.size, ctx.canvas.height - p.size);
      } else {
  ...
  constructor(particleIndex, settings, canvas) {
    ...
    if (velocity.hasOwnProperty("directionAngle")) {
      if (velocity.hasOwnProperty("directionAngleVariance")) {
        let angle = this.getRandomInRange(
          velocity.directionAngle - velocity.directionAngleVariance,
          velocity.directionAngle + velocity.directionAngleVariance,
        );
